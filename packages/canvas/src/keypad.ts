 ...
 * (`drawlist.ts:105-106` emits `x: col * atlas.cellWidth`). The renderer multiplies by whatever
 ...
 * `column()` the screen and the OIA use. That is what keeps the screenshot goldens
 * byte-reproducible: `fillText` would pull in a system font, and font rasterisation is the
 * machine-dependent thing that stops a golden reproducing -- the reason `drawlist.ts:60-68` gives
...
export function keypadRegion(atlas: AtlasGeometry, scheme: Scheme, y: number): KeypadRegion {
  ...
  // -- the same argument and the same pair `oiaCells` uses at `drawlist.ts:146-147`.
