...
| the TUI's `Ctrl-T` transfer form vs a live host | **NOT YET VERIFIED** — 41 tests cover the model, the renderer, the key routing and the engine, including that the command reaches the screen and Enter reaches the wire, but no file has crossed a real host through the form. Until it has, the honest claim is that the *transfer* is live-verified and the *form* is not |
